...
namespace CURSOIII.LINQ
{
    class Class1
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("compund from with racers and cars - " +
                    "retrieving Formula champions with a Ferrari");
            ...
            Console.WriteLine();
            // Compound From es una conversion del compilador donde
            // from() es convertida del metodo de extension SelectMany()
            // a continuacion se muestra como se usa con el metodo de extension
            ...
            {
                Console.WriteLine("Show all champions from Brazil ordered " +
                    "by wins descending");
                Console.WriteLine();
                var racers = Formula1.GetChampions()
            ...
            {
                Console.WriteLine("show the first 10 champions ordered by " +
                ...
                {
                    Console.WriteLine($"{racer.Country}: {racer.LastName}," +
                        $"{racer.FirstName}");
                }
            }
            SortMultiple();
            ...
            // No solo las propiedades del pais y numero de corredores
            // tambien una secuencia de los nombres de los pilotos.
            // Esta secuencia se asigna mediante la from / in clause.
            // (from r1 in g) asignada a la propiedad Racers.
            // La clausula from tambien obtiene todos los pilotos
            // y los ordena por apellido y crear un string por nombre y ap.
            ...
            {
                Console.WriteLine("Inner join with a LINQ query - " +
                    "combining drivers and constructors");
                var racers = from r in Formula1.GetChampions()
                             from y in r.Years
                             select new
                             {
            ...
            static void LeftOuterJoin()
            {
                ...
                var racersAndTeams = (from r in racers
                                      join t in teams on r.Year equals
                                      t.Year into rt
                ...
                foreach(var item in racersAndTeams)
                {
                    Console.WriteLine($"{item.Year}: " +
                        $"{item.Champion,-20} {item.Constructor}");
                }
            }
            LeftOuterJoin();
            Console.WriteLine();
            ...
            {
                int pageSize = 5;
                int numberPages = (int)Math.Ceiling(Formula1.GetChampions()
    ...
    public record Racer(string FirstName, string LastName,
        string Country, int Starts, int Wins,
        IEnumerable<int> Years, IEnumerable<string> Cars) :
        IComparable<Racer>, IFormattable
    {
        public Racer(string FirstName, string LastName,
            string Country, int Starts, int Wins)
            : this(FirstName, LastName, Country, Starts,
                  Wins, new int[] { }, new string[] { })
        { }
        ...
        public string ToString(string? format, IFormatProvider? formatProvider)
        {
            return format switch
            {
                ...
                "S" => Starts.ToString(),
                "W" => Wins.ToString(),
                "A" => $"{FirstName} {LastName}, country: {Country}, " +
